...
std::optional<BRD> BRDDao::queryBRD(const QString& bridgeNumber) {
	...
	if (query.exec() && query.next()) {
		return BRD(
			static_cast<BRD::BRDType>(query.value("designDrawings").toInt()),
			static_cast<BRD::BRDType>(query.value("designDoc").toInt()),
			static_cast<BRD::BRDType>(query.value("recordDrawings").toInt()),
			static_cast<BRD::BRDType>(query.value("acceptanceDoc").toInt()),
			static_cast<BRD::BRDType>(query.value("ADDoc").toInt()),
			static_cast<BRD::BRDType>(query.value("PMS").toInt()),
			static_cast<BRD::BRDType>(query.value("SIS").toInt()),
			static_cast<BRD::BRDType>(query.value("RRR").toInt()),
			query.value("otherArchive").toString(),
			static_cast<BRD::ArchiveType>(query.value("archiveType").toInt()),
			query.value("filingDate").toDateTime()
		);
	}
...
void BRDDao::updateBRD(const QString& bridgeNumber, const BRD& info) {
	auto sql = QString("UPDATE brd SET designDrawings = ?, designDoc = ?, recordDrawings = ?, \
acceptanceDoc = ?, ADDoc = ?, PMS = ?, SIS = ?, RRR = ?, otherArchive = ?, archiveType = ?, \
filingDate = ? WHERE bridgeNumber = ?;");
	DEBUG(sql);

	QSqlQuery query{};
	query.prepare(sql);
	query.addBindValue(static_cast<int>(info.getDesignDrawings()));
	query.addBindValue(static_cast<int>(info.getDesignDoc()));
	query.addBindValue(static_cast<int>(info.getRecordDrawings()));
	query.addBindValue(static_cast<int>(info.getAcceptanceDoc()));
	query.addBindValue(static_cast<int>(info.getADDoc()));
	query.addBindValue(static_cast<int>(info.getPMS()));
	query.addBindValue(static_cast<int>(info.getSIS()));
	query.addBindValue(static_cast<int>(info.getRRR()));
	query.addBindValue(info.getOtherArchive());
	query.addBindValue(static_cast<int>(info.getArchiveType()));
	query.addBindValue(info.getFilingDate());
	query.addBindValue(bridgeNumber);
